...
- Magnitudes of comets: `Elliptic` used the H-G system of asteroids for comets, which has no activity term, and `Parabolic` and `NearParabolic` calculated the distance to the earth from the geocentric position of the comet minus the heliocentric position of the earth, in two different frames. Against JPL Horizons' T-mag, 12P/Pons-Brooks at perihelion was 2.3 magnitudes too faint, C/2023 A3 2.4 and C/2025 A6 (Lemmon) 4.3 magnitudes. All three classes now use the new `CometPhotometry` trait: the total magnitude m = H + 5 log(delta) + K log(r), with r and delta from the same heliocentric position as the coordinates and the Sun of `_sunRectangularCoordinates()`. They now agree with Horizons within 0.001 magnitudes. `Elliptic::setCometParams()` is new; an `Elliptic` object with comet parameters uses the comet formula, one with `setHG()` the H-G system. `hasCometParams()` tells whether comet parameters were given.
- Magnitudes of asteroids: `Elliptic::magnitude()` mixed an equatorial and an ecliptical position for the distance to the earth and the phase angle, which made (433) Eros 0.17 magnitudes too faint at a phase angle of 60 degrees. It now agrees with the APmag of Horizons within 0.001 magnitudes.
- `astronomy:updateCometPhotometry` stored wrong values: when SBDB had no slope it took `n` from `orbit.elements`, which is the mean motion in degrees per day; text on aerith.net like "n = ..." or "phase ..." outside the m1 formula put unrelated numbers, julian days among them, into `n` and `phase_coeff`; the slope parameter G of the H-G system was stored as a phase coefficient in magnitudes per degree; and K2 and M2 of SBDB, which describe the nucleus, were used for the total magnitude. The command now uses the m1 formula of aerith.net, or M1 and K1 of SBDB. An H without a K, of an asteroid-like light curve, is stored with K = 5, which is the H-G system without its phase term. Values outside a plausible range (H from -10 to 30, K from 1 to 60, a phase coefficient from 0 to 0.1 magnitudes per degree) are not stored.

Changed:

- The second argument of `setCometParams()` is the coefficient K of log(r), as `astronomy:updateCometPhotometry` stores it and as aerith.net (m1 = H + 5 log d + K log r) and JPL (K1) publish it. Its default is now 10 instead of 4: 4 is the usual activity exponent n, and K = 2.5 n. The asymmetric values before and after the perihelion are K values as well.
- `Elliptic`, `Parabolic` and `NearParabolic` calculate their heliocentric position in one place, `_heliocentricRectangularCoordinates()`, used for both the coordinates and the magnitude. The coordinates do not change.
